...
isdir(datadir("glycosylationEvariation2D")) ? nothing : mkdir(datadir("glycosylationEvariation2D"))
...
record(fig, datadir("glycosylationEvariation2D","c_against_x.mp4"), 1:length(sol.t); framerate=10) do i
...
save(datadir("glycosylationEvariation2D", "finalνVsM.png"), fig)
...
record(fig, datadir("glycosylationEvariation2D", "Mvsν.mp4"), 1:length(sol.t); framerate=10) do i
    uInternal2D .= reshape(sol.u[i][ghostVertexMask], (Nx, Nν))
    M[] .= sum(uInternal2D, dims=1)[1,:]
    M[] = M[]
end
...
# record(fig, datadir("glycosylationEvariation2D", "LargeNuTimeScan.mp4"), 1:length(sol.t); framerate=10) do i
...
# record(fig, datadir("glycosylationEvariation2D", "NuProfileAtxyOverTime.mp4"), 1:length(sol.t); framerate=10) do i
#     uInternal3D .= reshape(sol.u[i][ghostVertexMask], (Nx, Ny, Nν))
...
# record(fig, datadir("glycosylationEvariation2D", "xνOverTimeAty.mp4"), 1:length(sol.t); framerate=10) do i
