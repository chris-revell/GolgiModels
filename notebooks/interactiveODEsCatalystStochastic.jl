...
discreteprob  = DiscreteProblem(system, u₀Map, (0.0,Inf), p)
...
integ = init(jumpProblem, SSAStepper())

# Function to update figure based on system iteration
function animstep!(integ,stochasticCisObservable,stochasticMedObservable,stochasticTraObservable,nMax)
    step!(integ, 10.0)
    @show integ.t
...
xlims!(axCis,(0.0,10.0))
xlims!(axMed,(0.0,10.0))
xlims!(axTra,(0.0,10.0))
...
stochasticCisObservable = Observable(u₀[1:nMax].*volume)
# Set up observable objects for med results
stochasticMedObservable = Observable(u₀[1+nMax:2*nMax].*volume)
# Set up observable objects for tran results
stochasticTraObservable = Observable(u₀[1+2*nMax:3*nMax].*volume)
...
on(run.clicks) do clicks
    @async while isrunning[]       
        ...
        animstep!(integ,stochasticCisObservable,stochasticMedObservable,stochasticTraObservable,nMax)        
        sleep(0.01) # yield()
    end
end
