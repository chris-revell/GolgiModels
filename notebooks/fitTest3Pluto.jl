...
# ╔═╡ b2147e1b-be81-4898-acc0-daf5e07b7d6d
begin
	# using CairoMakie
...
# ╔═╡ 98eeb2e9-1b00-4483-aaa6-1bd1d7b03dc8
expValSlider = @bind expVal PlutoUI.Slider(1.0:0.1:5.0)

# ╔═╡ 5bb14fcf-5eaa-45d9-b9cf-2dd1500c579d
nChainsSlider = @bind nChains PlutoUI.Slider(1:1:4)

# ╔═╡ 339ce018-1086-4fc0-954d-1374ff568d4b
iterationsSlider = @bind iterations PlutoUI.Slider(100:1000:100000)

# ╔═╡ 374c76a0-51ab-4485-9e95-c779ed756082
@model function fitmodel(data, prob)
    # Prior
    σ ~ InverseGamma(2, 3)
    # k₂ ~ truncated(Normal(0.3,0.2),0.2,0.4)
    k₃ ~ truncated(Normal(0.5,0.2),0.4,0.6)
    k₄ ~ truncated(Normal(0.3,0.2),0.0,1.2)
    # V ~  truncated(Normal(300,100),100.0,500.0)
    # Likelihood
    p = [1.0,0.3,k₃,k₄]
...
# ╔═╡ 1b1d92f7-4825-4a6d-a229-432b97059748
begin
...
# ╔═╡ 771ed516-fb66-4437-80ad-09b3298da723
pODE .= Pair.(collect(k),zeros(Float64,4))

# ╔═╡ b9e31a8d-a351-4cd2-b661-14ded8fe18ac
begin
	dummyData = last.(sort(collect(counter(ceil.(Int64,rand(Exponential(expVal),1000)))),by=first))
	# dummyData = last.(sort(collect(counter(ceil.(Int64,1.0./rand(Uniform(0.01,1),1000)))),by=first))
	if length(dummyData)<nMax
	    append!(dummyData, zeros(Int64,nMax-length(dummyData)))
	else
	    dummyData = dummyData[1:nMax]
	end
	plot(collect(1:nMax),dummyData,size=(300,300))
end

# ╔═╡ 42f01bba-00a5-4d19-a350-1ee878cdd1f4
begin
	...
	chain = sample(model, NUTS(), MCMCSerial(), iterations, nChains)
	chainDF = DataFrame(chain)
end

# ╔═╡ ad4189f1-fc25-4464-81d6-40f57a9015a6
nMaxSlider = @bind nMax PlutoUI.Slider(10:1:50)

# ╔═╡ 19219fdf-a22f-41f5-be99-4573e291d5cc
begin
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "0975282dfe8738a4ea4c5db1f0e1de74f9964fb1"
