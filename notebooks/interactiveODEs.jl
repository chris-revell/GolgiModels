...
function deterministicModel!(du,u,p,t)
    ...
    du[1] = ks[1] - ks[4]*u[1] + ks[5]*u[1+nMax] - ks[2]*u[1]^2 - ks[2]*u[1]*sum(@view u[2:nMax-1]) + 2*ks[3]*u[2] + ks[3]*sum(@view u[3:nMax])
    ...
    for n=3:nMax-1
        du[n] = ks[2]*u[n-1]*u[1] + ks[3]*u[n+1] - ks[2]*u[n]*u[1] - ks[3]*u[n]
    end
    ...
    du[1+nMax] = ks[4]*u[1] - ks[5]*u[1+nMax] + ks[9]*u[1+2*nMax] - ks[8]*u[1+nMax] - ks[6]*u[1+nMax]^2 - ks[6]*u[1+nMax]*sum(@view u[2+nMax:2*nMax-1]) + 2*ks[6]*u[2+nMax] + ks[7]*sum(@view u[3+nMax:2*nMax])
    ...
    du[1+2*nMax] = ks[8]*u[1+nMax] - ks[9]*u[1+2*nMax] - ks[12]*u[1+2*nMax] - ks[10]*u[1+2*nMax]^2 - ks[10]*u[1+2*nMax]*sum(@view u[2+2*nMax:3*nMax-1]) + 2*ks[11]*u[2+2*nMax] + ks[11]*sum(@view u[3+2*nMax:3*nMax])
