...
function deterministicModel!(du,u,p,t)
    ...
    du[1,1] = k₀ - k₃*u[1,1] + k₄*u[1,2] - (2*k₁*u[1,1]^2)/2 - k₁*u[1,1]*sum(u[2:nMax-1,1]) + 2*k₂*u[2,1] + k₂*sum(u[3:nMax,1])
    ...
    du[1,2] = k₃*u[1,1] - k₄*u[1,2] + k₈*u[1,3] - k₇*u[1,2] - (2*k₅*u[1,2]^2)/2 - k₅*u[1,2]*sum(u[2:nMax-1,2]) + 2*k₆*u[2,2] + k₆*sum(u[3:nMax,2]) 
    ...
    du[1,3] = k₇*u[1,2] - k₈*u[1,3] - k₁₁*u[1,3] - (2*k₉*u[1,3]^2)/2 - k₉*u[1,3]*sum(u[2:nMax-1,3]) + 2*k₁₀*u[2,3] + k₁₀*sum(u[3:nMax,3])
